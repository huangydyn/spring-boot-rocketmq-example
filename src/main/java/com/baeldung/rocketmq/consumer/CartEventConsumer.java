...
public class CartEventConsumer {
    ...
    public class CardItemAddConsumer implements RocketMQListener<MessageExt> {
        public void onMessage(MessageExt messageExt) {
            ...
            try {
                addItemEvent = new ObjectMapper().readValue(new String(messageExt.getBody()), CartItemEvent.class);
            } catch (Exception e) {
                throw new RuntimeException(e.getMessage());
            }

            logger.info("[Receive Msg] item offset {} ，body {}", messageExt.getCommitLogOffset(), addItemEvent);
